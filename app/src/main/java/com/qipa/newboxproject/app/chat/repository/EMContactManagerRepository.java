...
public class EMContactManagerRepository extends BaseEMRepository{
        ...
        return new NetworkBoundResource<List<EaseUser>, List<EaseUser>>() {
                    ...
                    try {
                        ...
                        if(usernames !=null && usernames.size() > 0){
                            ...
                            if(getUserDao() != null) {
                                ...
                                if(exitUsers != null){
                                    for(String userId:exitUsers){
                                        if(ChatHelper.Companion.getInstance().isCurrentUserFromOtherDevice(userId)){
                            ...
                            if(exitUsers == null || exitUsers.size() == 0){
                                ...
                                //用户属性过期的好友
                                List<String> timeOutUsers = getUserDao().loadTimeOutFriendUser(DemoModel.Companion.getUserInfoTimeOut(),System.currentTimeMillis());
            ...
            protected void saveCallResult(List<EaseUser> items) {
                if(getUserDao() != null) {
                    ...
                    getUserDao().insert(EmUserEntity.Companion.parseList(items));
                }
            }
            ...
            public void onSuccess(Map<String, EMUserInfo> value) {
                ...
                if(users != null && !users.isEmpty()) {
                    for (EaseUser user : users) {
                        ...
                        if(ChatHelper.Companion.getInstance().isCurrentUserFromOtherDevice(user.getUsername())){
    ...
    public LiveData<Resource<Boolean>> deleteContact(String username) {
        return new NetworkOnlyResource<Boolean>() {
            @Override
            protected void createCall(@NonNull ResultCallBack<LiveData<Boolean>> callBack) {
                ChatHelper.Companion.getInstance().getModel().deleteUsername(username, true);
                getContactManager().aysncDeleteContact(username, new EMCallBack() {
                    @Override
                    public void onSuccess() {
                        ChatHelper.Companion.getInstance().deleteContact(username);
    ...
    public LiveData<Resource<EaseUser>> getUserInfoById(final String username, boolean mIsFriend) {
        return new NetworkBoundResource<EaseUser, EaseUser>() {
            ...
            protected void createCall(ResultCallBack<LiveData<EaseUser>> callBack) {
                ...
                if( ChatHelper.Companion.getInstance().isCurrentUserFromOtherDevice(username)) {
            ...
            protected void saveCallResult(EaseUser item) {
                if(mIsFriend) {
                    getUserDao().insert(EmUserEntity.Companion.parseParent(item));
                }
            }
        }.asLiveData();
    }
