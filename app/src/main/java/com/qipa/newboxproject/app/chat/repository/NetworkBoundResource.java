...
import com.qipa.newboxproject.app.chat.net.Result;
...
public abstract class NetworkBoundResource<ResultType, RequestType> {
                    ...
                    result.addSource(apiResponse, response-> {
                        ...
                        if(response != null) {
                            ...
                            if(response instanceof Result) {
                                int code = ((Result) response).getCode();
                            ...
                            mThreadManager.runOnIOThread(() -> {
                                ...
                                mThreadManager.runOnMainThread(() ->
                                        result.addSource(safeLoadFromDb(), newData -> {
                                            setValue(Resource.Companion.success(newData));
                                        }));
                            });
    ...
    private void fetchFailed(int code, LiveData<ResultType> dbSource, String message) {
        onFetchFailed();
        try {
            result.addSource(dbSource, newData -> setValue(Resource.Companion.error(code, message, newData)));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
