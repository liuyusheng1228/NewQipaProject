...
class YanDeleteEditText : AppCompatEditText {
    /*
     * 定义属性变量
     * */
    private var mPaint // 画笔
            : Paint? = null
    private var ic_deleteResID // 删除图标 资源ID
            = 0
    private var ic_delete // 删除图标
            : Drawable? = null
    private var delete_x = 0
    private var delete_y = 0
    private var delete_width = 0
    private var delete_height // 删除图标起点(x,y)、删除图标宽、高（px）
            = 0
    private var ic_left_clickResID = 0
    private var ic_left_unclickResID // 左侧图标 资源ID（点击 & 无点击）
            = 0
    private var ic_left_click: Drawable? = null
    private var ic_left_unclick // 左侧图标（点击 & 未点击）
            : Drawable? = null
    private var left_x = 0
    private var left_y = 0
    private var left_width = 0
    private var left_height // 左侧图标起点（x,y）、左侧图标宽、高（px）
            = 0
    private var cursor // 光标
            = 0

    // 分割线变量
    private var lineColor_click = 0
    private var lineColor_unclick // 点击时 & 未点击颜色
            = 0
    private var color = 0
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        // 获取控件资源
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.YanDeleteEditText)
        /**
         * 初始化左侧图标（点击 & 未点击）
         */
        ...
        // a. 点击状态的左侧图标
        // 1. 获取资源ID
        ic_left_clickResID = typedArray.getResourceId(
            R.styleable.YanDeleteEditText_ic_left_click,
            R.drawable.ic_left_click
        )
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
        ic_left_click = resources.getDrawable(ic_left_clickResID)
        // 3. 设置图标大小
        ...
        // b. 未点击状态的左侧图标
        // 1. 获取资源ID
        ic_left_unclickResID = typedArray.getResourceId(
            R.styleable.YanDeleteEditText_ic_left_unclick,
            R.drawable.ic_left_unclick
        )
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
        // 3. 设置图标大小（此处默认左侧图标点解 & 未点击状态的大小相同）
        ic_left_unclick = resources.getDrawable(ic_left_unclickResID)
        ...
        // 1. 获取资源ID
        ic_deleteResID =
            typedArray.getResourceId(R.styleable.YanDeleteEditText_ic_delete, R.drawable.delete)
        // 2. 根据资源ID获取图标资源（转化成Drawable对象）
        ...
        // 1. 设置画笔
        mPaint = Paint()
        mPaint!!.strokeWidth = 2.0f // 分割线粗细

        // 2. 设置分割线颜色（使用十六进制代码，如#333、#8e8e8e）
        val lineColorClick_default =
            context.resources.getColor(R.color.mainColor) // 默认 = 蓝色#1296db
        val lineColorunClick_default =
            context.resources.getColor(R.color.gray_61) // 默认 = 灰色#9b9b9b
        lineColor_click = typedArray.getColor(
            R.styleable.YanDeleteEditText_lineColor_click,
            lineColorClick_default
        )
        lineColor_unclick = typedArray.getColor(
            R.styleable.YanDeleteEditText_lineColor_unclick,
