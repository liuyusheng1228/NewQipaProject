...
import java.io.File
import com.qipa.jetpackmvvm.network.NetworkUtil.url
...
class GlideUtils {
    ...
    companion object {
        /*** 占位图  */
        var placeholderImage: Int = R.drawable.af_quick_pass

        /*** 错误图  */
        var errorImage: Int = R.drawable.af_quick_pass
        ...
        fun loadRoundCircleImage(
            ...
            val options = RequestOptions.bitmapTransform(
                MultiTransformation(
                    CenterCrop(),
                    RoundedCornersTransformation(SizeUtils.px2dp(radius), 0, type)
                )
            )
                .placeholder(placeholderImage) //占位图
                .error(errorImage) //错误图
