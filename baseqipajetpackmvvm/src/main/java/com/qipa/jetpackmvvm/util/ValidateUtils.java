...
//^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}$
	...
	public static boolean isIDCard(String IDCardNo) {
		...
		if (IDCardNo.length() != 15 && IDCardNo.length() != 18) {
			...
			AppLogMessageMgr.INSTANCE.e("AppValidationMgr-->>isIDCard", errmsg);
			return false;
		}
		...
		if (DateUtils.Companion.getDateIsTrue(strYear, strMonth, strDay) == false) {
			errmsg = "身份证生日无效";
			AppLogMessageMgr.INSTANCE.e("AppValidationMgr-->>isIDCard", errmsg);
			return false;
		}
		...
		try {
			...
			e.printStackTrace();
			errmsg = "身份证生日不在有效范围";
			AppLogMessageMgr.INSTANCE.e("AppValidationMgr-->>isIDCard", errmsg  + e.getMessage());
			return false;
		} catch (java.text.ParseException e1) {
			e1.printStackTrace();
			errmsg = "身份证生日不在有效范围";
			AppLogMessageMgr.INSTANCE.e("AppValidationMgr-->>isIDCard", errmsg + e1.getMessage());
			return false;
		}
